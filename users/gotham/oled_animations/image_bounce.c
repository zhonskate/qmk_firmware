...
// clang-format on
static const uint8_t* image_data = dvd_logo;
...
void oled_init_image_bounce(void) {
    ...
    logo_x    = random8_max(OLED_FINAL_WIDTH - IMAGE_WIDTH);
    logo_y    = random8_max(OLED_FINAL_HEIGHT - IMAGE_HEIGHT);
    move_up   = (random8() % 2 == 0);
