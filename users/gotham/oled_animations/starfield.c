...
static uint16_t center_x            = OLED_FINAL_WIDTH / 2;
static uint16_t center_y            = OLED_FINAL_HEIGHT / 2;
...
void set_starfield_center(void) {
    center_x = random8_max(OLED_FINAL_WIDTH);
    center_y = random8_max(OLED_FINAL_HEIGHT);
}
