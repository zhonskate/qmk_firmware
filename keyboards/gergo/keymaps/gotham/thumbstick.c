...
void thumbstick_init(void) {
    ...
    thumbstick_state.config.mode           = THUMBSTICK_MODE_OFF;
...
int16_t thumbstick_get_component(uint8_t pin) {
    uint16_t analogValue = adc_read(pin);
    // Compute direction
...
void thumbstick_mode_cycle(bool reverse) {
    update_keycode_status(KC_UP, thumbstick_state.direction.up, false);
    update_keycode_status(KC_DOWN, thumbstick_state.direction.down, false);
    update_keycode_status(KC_LEFT, thumbstick_state.direction.left, false);
    update_keycode_status(KC_RIGHT, thumbstick_state.direction.right, false);
...
void pointing_device_task(void) {
    if (thumbstick_state.config.mode != THUMBSTICK_MODE_OFF) {
